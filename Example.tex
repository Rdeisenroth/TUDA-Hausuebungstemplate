...
\begin{task}[points=auto,solution=true]{Alternativer style}
    ...
    \begin{subtask}[title={Irgend son Graph},points=2]
        ...
        \begin{solution}
            ...
            % Check: \IfNestedContainsTF{3={a)=3,b)=2,},4=2}{3,c)}{true}{false}

            % asd: \nestedget{3={a)=3,b)=2,},4=2}{3,b)}

            --Testcode--\par
            \ExplSyntaxOn
            %
            \tl_new:N \l_lel_tl
            % \tl_set:Nn \l_lel_tl {3={a)=3,b)=2,c)={1=2,2=3}},4={a)=3}}
            \tl_set:Nn \l_lel_tl {3={a)=3,b)=2,c)={1=2,2=3}},4={a)=3},a=3}
            \rubos_prop_nested_set:Nnn \l_lel_tl {5,c),2} {9}
            \rubos_prop_nested_set:Nnn \l_lel_tl {5,d),5} {8}
            % \tl_use:N \l_lel_tl\par
            tl: \rubos_print_nested_tl:n {\l_lel_tl}
            % \par get: \nestedget{\l_lel_tl}{3,c),2}\par
            \clearpage
            % a:
            % \prop_clear_new:N \l_tmpa_subtask_prop
            % \prop_set_from_keyval:Nn \l_tmpa_subtask_prop {
            %     title={\l_ptxcd_ex_title_tl},
            %     credit={\l_ptxcd_ex_subtask_credit_tl},
            %     points={\fp_use:N \l_ptxcd_ex_subtask_points_fp},
            %     thesubtask=\thesubtask,
            %     subtaskformat=\subtaskformat,
            % }
            % \tl_clear_new:N \l_tmpa_subtask_prop_tl
            % \prop_map_inline:Nn \l_tmpa_subtask_prop {
            %     \tl_put_right:Nn \l_tmpa_subtask_prop_tl {#1={#2},}
            % }
            % \l_tmpa_subtask_prop_tl
            % \exp_args:Nx \rubos_print_nested_tl:n {\l_tmpa_subtask_prop_tl}
            % \exp_args:Nx \rubos_print_nested_tl:n {\l_tmpa_subtask_prop_tl}
            \ExplSyntaxOff
        \end{solution}
    \end{subtask}
...
\begin{task}[points=0]{Punktetabellen (WIP)}
    ...
    \begin{table}[ht]
        ...
        \begin{tabularx}{\textwidth}{|X|c|c|}
            ...
            \midrule\midrule
            \mapTasks*{%
            \textbf{\taskformat{}:~\getTaskTitle{\the\value{task}}}                                       & \getPoints{\thetask}                                 & \IfSolutionT{\fatsf{\getPoints{\thetask}}}\IfSubtasksTF{\mapSubtasks{      \\%
            \rowcolor{\thepagecolor}\thesubtask{}~\getSubTaskTitle{\the\value{task}}{\the\value{subtask}} & \getSubPoints{\the\value{task}}{\the\value{subtask}} & \IfSolutionT{\fatsf{\getSubPoints{\the\value{task}}{\the\value{subtask}}}} %
                    }
            }{}                                                                                                                                                                                                                               \\
                \midrule\midrule
            }
...
%     % \tl_clear_new:N \l__ptxcd_task_properties_loaded_tl
%     % \tl_set_eq:NN \l__ptxcd_task_properties_loaded_tl \g__ptxcd_task_properties_loaded_tl
%     % \rubos_prop_nested_set:Nnn \g__ptxcd_task_properties_loaded_tl {3,points} {9}
%     % \tl_gset_eq:NN \g__ptxcd_task_properties_loaded_tl \l__ptxcd_task_properties_loaded_tl
%     }\par
